...
A future wrapper skill may invoke the installed CLI once from the user's controlling session. Make it explicitly invoked and unavailable to worker tasks; otherwise a worker reviewing code could rediscover the wrapper and recursively launch the whole suite. Keep the orchestration executable outside implicit worker skill discovery.
